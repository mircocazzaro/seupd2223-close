...
called by the main class \textit{CloseSearchEngine} of our system. The \textit{Analyzer} class is instantiated before all, as it is used both from the two main branches stated before. Here many other Lucene and Solr tool are istantiated, as long as this component
...
This is the process of ranking documents retrieved by the search function of the \textit{Searcher} (\ref{searcher_subsec}).
For this task, we used sentence transformers~\cite{sentence-transformers}, a Python framework for state-of-the-art sentence, text and image embeddings.
We experimented with different models, but the one that gave us the best results was \textit{all-MiniLM-L6-v2}~\cite{huggingfaceallminilml6v2}, a model that aims to train sentence embedding models on very large sentence level datasets using a self-supervised contrastive learning objective. It maps sentences and paragraphs to a 384 dimensional dense vector.
We load the \textit{all-MiniLM-L6-v2} model to create a \textit{SentenceTransformer} object, which is used to create embeddings for the documents and query retrieved by the search function.
For calculating the similarity, we used the \textit{cosine similarity} formula, which is the most used formula for calculating the similarity between two vectors.
The formula is:
\begin{equation}
    similarity = \frac{t \cdot d\_i}{\|t\| \|d\_i\|}\label{eq:similarity_equation}
\end{equation}
where $t$ is the query vector and $d\_i$ is the i'th document vector.
\newline
Then we try to merge this score with the BM25 score to achieve better rank for the documents.
We tried different approaches, but the one that gave us the best results was the following:
\begin{equation}
    rank = BM25\_score * similarity\label{eq:rank_equation}
\end{equation}
...
After all, sort the documents by the new rank and return them.
