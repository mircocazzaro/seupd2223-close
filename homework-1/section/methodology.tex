...
The workflow of our system, starting from the train collection provided by \ac{CLEF}, is as follows:
\begin{enumerate}
...
After different tries, we decided to use only the function above for removing noises, which gave an improving in MAP of +0.5\%, together with another one used to remove \textit{JavaScript} codes, which by itself gave +3\% on MAP. \\
